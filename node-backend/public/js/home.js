document.addEventListener('DOMContentLoaded', function () {
  ...
  fetchWithCredentials('/personalDetails')
    ...
    .catch(error => {
      devLog('Error fetching personal details:', error);
  ...
  fetchWithCredentials('/getBudgetDetails')
    ...
    .then(data => {
      if (data.success && Array.isArray(data.budgets)) {
        ...
        devLog('Unexpected data format or failed request.');
      }
    })
    .catch(error => {
      devLog('Error fetching budget details:', error);
    });

  // Save budget button handler
  document.getElementById('save-budget-btn').addEventListener('click', function () {
    ...
    for (const [category, amountRaw] of Object.entries(budgetDetails)) {
      ...
      promises.push(
        ...
        fetchWithCredentials('/saveBudgetDetails', {
          ...
          .catch(error => {
            devLog('Error saving budget details:', error);
          })
      );
    }
    ...
    Promise.all(promises)
      ...
      .catch(error => {
        devLog('Error processing budget details:', error);
      });
  });

  // Fetch current month expenses for chart
  function fetchCurrentMonthExpenses() {
    fetchWithCredentials('/currentMonthExpenses')
      ...
      .catch(error => {
        devLog('Error fetching current month expenses:', error);
      });
  }
